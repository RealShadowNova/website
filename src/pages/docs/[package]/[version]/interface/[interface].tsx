...
const Interface: Component = () => {
  ...
  const [parser, { refetch }] = createResource(async () => {
    ...
    const parser = projectParser.interfaces.find((interfaceParser) => interfaceParser.name === params.interface);

    if (parser === undefined) {
      throw new Error(`Interface '${params.interface}' not found.`);
    }
    ...
    <PageLayout>
      <div class='flex flex-col gap-4 xl:mr-64 h-full'>
        <Show
          ...
          keyed
        >
          {(parser) => <InterfaceModel interface={parser} />}
        </Show>
      </div>
