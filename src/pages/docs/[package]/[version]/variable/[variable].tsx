...
const Variable: Component = () => {
  ...
  const [parser, { refetch }] = createResource(async () => {
    ...
    const parser = projectParser.variables.find((variableParser) => variableParser.name === params.variable);

    if (parser === undefined) {
      throw new Error(`Type alias '${params.variable}' not found.`);
    }
    ...
    <PageLayout>
      <div class='flex flex-col gap-4 h-full'>
        <Show
          ...
          keyed
        >
          {(parser) => <VariableModel variable={parser} />}
        </Show>
      </div>
    </PageLayout>
  );
